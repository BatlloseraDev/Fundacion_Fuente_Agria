...
export const ActividadCard = ({ actividad }: Props) => {
    const { id, title, description, date, category, imageUrl, colorClass } = actividad;
        ...
        <article className="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
            <div className="row g-0">
                <div className="col-md-5 col-lg-4 position-relative">
                    {/* Uso los template strings para inyectar la clase de color dinámica */}
                    <div className={`bg-${colorClass} bg-opacity-10 h-100 d-flex align-items-center justify-content-center`} style={{ minHeight: '250px' }}>
                        <img src={imageUrl} alt={title} className="img-fluid h-100" style={{ objectFit: 'cover', width: '100%' }} />
                    </div>
                ...
                <div className="col-md-7 col-lg-8">
                    <div className="card-body p-4 p-lg-5 d-flex flex-column h-100">
                        <div className="mb-2">
                            <span className={`text-${colorClass} fw-bold text-uppercase small tracking-wide`}>
                                {category}
                            </span>
                        </div>
                        ...
                        <div className="mt-auto">
                            <Link to={`/actividades/${id}`} className={`btn btn-outline-${colorClass} rounded-pill px-4`}>
