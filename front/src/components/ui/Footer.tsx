...
import type { FooterConfig } from '../../admin/types/admin.types';
import { getFooterConfig } from '../../admin/services/admin.service';

// ── Config por defecto (mientras carga o si falla la API) ─────────────────────
...
const AvisoLegal = () => (
  <div className="small text-muted" style={{ lineHeight: 1.7 }}>
    ...
    <p>El presente Aviso Legal regula el acceso y uso del sitio web de Fundación Fuente Agria. El acceso al sitio implica la aceptación plena y sin reservas de las presentes condiciones.</p>
    <h6 className="fw-bold text-dark mt-3">3. Propiedad intelectual e industrial</h6>
    <p>Los contenidos del sitio web (textos, imágenes, logotipos, diseño, código fuente, etc.) son propiedad de Fundación Fuente Agria o de sus colaboradores, y están protegidos por la legislación española e internacional sobre propiedad intelectual. Queda prohibida su reproducción, distribución o comunicación pública sin autorización expresa.</p>
    <h6 className="fw-bold text-dark mt-3">4. Exclusión de responsabilidad</h6>
    <p>Fundación Fuente Agria no se responsabiliza de los daños o perjuicios que pudieran derivarse del acceso o uso del sitio web, ni de la interrupción del servicio por causas ajenas a su voluntad.</p>
    <h6 className="fw-bold text-dark mt-3">5. Legislación aplicable y jurisdicción</h6>
    <p>Las presentes condiciones se rigen por la legislación española. Para la resolución de cualquier controversia, las partes se someten a los Juzgados y Tribunales de Ciudad Real, salvo que la normativa establezca otro fuero imperativo.</p>
  </div>
);
...
const PoliticaPrivacidad = () => (
  <div className="small text-muted" style={{ lineHeight: 1.7 }}>
    ...
    <ul>
      <li><strong>Gestión de usuarios registrados</strong> — base legal: ejecución del contrato (art. 6.1.b RGPD).</li>
      ...
      <li><strong>Envío de comunicaciones relacionadas con sus pedidos</strong> — base legal: interés legítimo (art. 6.1.f RGPD).</li>
    ...
    <p>Los datos se conservarán durante el tiempo necesario para cumplir la finalidad para la que fueron recogidos y, en su caso, durante los plazos legalmente exigidos (p. ej., 5 años para obligaciones fiscales según la Ley General Tributaria).</p>
    <h6 className="fw-bold text-dark mt-3">4. Destinatarios</h6>
    <p>No se cederán datos a terceros salvo obligación legal. No realizamos transferencias internacionales de datos.</p>
    <h6 className="fw-bold text-dark mt-3">5. Derechos de los interesados</h6>
    <p>Puede ejercer sus derechos de <strong>acceso, rectificación, supresión, oposición, limitación del tratamiento y portabilidad</strong> enviando un escrito a contacto@fuenteagria.org, adjuntando copia de su DNI. Si considera que el tratamiento no se ajusta a la normativa vigente, puede presentar una reclamación ante la <strong>Agencia Española de Protección de Datos</strong> (www.aepd.es).</p>
    <h6 className="fw-bold text-dark mt-3">6. Autenticación con Google</h6>
    <p>Si opta por autenticarse mediante su cuenta de Google, Google LLC actúa como encargado del tratamiento para la verificación de identidad. El tratamiento de sus datos por parte de Google se rige por su propia política de privacidad.</p>
  </div>
);
...
const PoliticaCookies = () => (
  <div className="small text-muted" style={{ lineHeight: 1.7 }}>
    ...
    <p>Las cookies son pequeños archivos de texto que un sitio web almacena en su dispositivo cuando lo visita. Permiten que el sitio recuerde sus acciones y preferencias durante un periodo de tiempo.</p>
    ...
    <p className="mt-2">Este sitio <strong>no utiliza cookies de seguimiento, publicidad ni analítica de terceros</strong>. El almacenamiento del token de sesión es estrictamente necesario para el funcionamiento del servicio y, conforme al art. 22.2 de la LSSI-CE, no requiere consentimiento previo.</p>
    <h6 className="fw-bold text-dark mt-3">Cómo eliminar la sesión</h6>
    <p>Puede cerrar sesión en cualquier momento desde el menú de usuario, lo que eliminará el token almacenado. También puede borrar el almacenamiento local de su navegador desde la configuración de privacidad.</p>
  </div>
);
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div className="modal-content border-0 rounded-4 shadow-lg">
          ...
          <div className="modal-footer border-0 px-4 pb-4 pt-2">
            <button type="button" className="btn btn-dark rounded-pill px-4" onClick={onClose}>Cerrar</button>
          </div>
        </div>
      </div>
    </div>
...
export const Footer = () => {
  ...
  useEffect(() => {
    getFooterConfig().then(setFooterConfig).catch(() => {});
    const refresh = () => getFooterConfig().then(setFooterConfig).catch(() => {});
    window.addEventListener('footer-config-updated', refresh);
    return () => window.removeEventListener('footer-config-updated', refresh);
  }, []);

  const { description, contact, collaborators } = footerConfig;
        ...
        <div className="container py-5">
          <div className="row g-4 justify-content-between">
            ...
            <div className="col-lg-3">
              <div className="d-flex align-items-center gap-3 mb-2">
                <img src="/imgs/Logo_FFA.png" alt="Logo Fundación Fuente Agria"
            ...
            {/* Columna 3: Colaboradores (solo si hay) */}
              ...
              <div className="col-lg-2">
                ...
                <div className="d-flex flex-wrap gap-3 align-items-center footer-logos mt-2">
                  {collaborators.map((col) => (
                    col.imageUrl
                      ? <img key={col.id} src={col.imageUrl} className="img-fluid"
                          alt={col.name} style={{ maxHeight: '40px', maxWidth: '80px', objectFit: 'contain' }} />
                      : <span key={col.id} className="text-muted small">{col.name}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="col-lg-2">
              ...
              <ul className="list-unstyled small mb-0" style={{ lineHeight: 2.2 }}>
                {(['aviso', 'privacidad', 'cookies'] as Exclude<LegalType, null>[]).map((key) => (
                  <li key={key}>
            ...
            {/* Columna 5: Gestión (solo roles) */}
            {(isEditor || isAdmin) && (
              <div className="col-lg-2">
                <h3 className="h6 fw-semibold">Gestión</h3>
                <div className="d-flex flex-column gap-2 mt-2">
                  {isEditor && !editMode && (
                    <button className="btn btn-sm btn-primary w-100 shadow-sm rounded-pill"
                  ...
                  {isAdmin && (
                    <button className="btn btn-sm btn-outline-secondary w-100 rounded-pill"
          ...
          <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 text-muted small">
            ...
            <div className="d-flex gap-3">
              {(['aviso', 'privacidad', 'cookies'] as Exclude<LegalType, null>[]).map((key) => (
                <button key={key} className="btn btn-link p-0 text-muted text-decoration-none small"
                  onClick={() => setLegalOpen(key)}>
                  {key === 'aviso' ? 'Aviso legal' : key === 'privacidad' ? 'Privacidad' : 'Cookies'}
                </button>
              ))}
            </div>
          </div>
