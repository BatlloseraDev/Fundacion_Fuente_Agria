...
export const ModalEdicion = ({ isOpen, onClose, onSave, tituloModal, valorInicial, estiloInicial = 'Normal' }: ModalEdicionProps) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setEstilo(estiloInicial || 'Normal');
            document.body.style.overflow = 'hidden';
        } else {
            document.body.style.overflow = '';
        }
        return () => { document.body.style.overflow = ''; };
    }, [isOpen, valorInicial, estiloInicial]);
                    ...
                    <div className="modal-content border-0 shadow-lg rounded-4 p-3">
                        ...
                        <div className="modal-body">
                            <div className="mb-4">
                                ...
                                <textarea className="form-control rounded-3" rows={4} value={valor} onChange={(e) => setValor(e.target.value)} placeholder="Escribe el texto aquí..." autoFocus style={{ resize: 'none' }} />
                            </div>
                            <div className="mb-2">
                                <label className="form-label text-secondary small fw-semibold mb-1">Estilo (Opcional)</label>
                                <select className="form-select rounded-3 text-secondary" value={estilo} onChange={(e) => setEstilo(e.target.value)}>
                        ...
                        <div className="modal-footer border-top-0 pt-0 d-flex gap-2 justify-content-end">
                            ...
                            <button className="btn btn-primary rounded-pill px-4 fw-medium" onClick={() => onSave(valor, estilo)}>Guardar Cambios</button>
                        </div>
                    </div>
                </div>
            </div>
